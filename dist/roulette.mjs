...
function loadState(){
  try{
    ...
    return {inventory,payoutTray,stacks,previousStacks,selectedChip,dealerChip,dealerTips:Number.isFinite(saved?.dealerTips)&&saved.dealerTips>=0?saved.dealerTips:0,history:Array.isArray(saved?.history)?saved.history.slice(0,12):Array.isArray(legacy?.history)?legacy.history.slice(0,12):[],tableCollapsed:Boolean(saved?.tableCollapsed),halfCredit:saved?.halfCredit===.5?.5:0};
  }catch{return {inventory:CHIP_VALUES.map(()=>0),payoutTray:[],stacks:new Map(),previousStacks:new Map(),selectedChip:1,dealerChip:'max',dealerTips:0,history:[],tableCollapsed:false,halfCredit:0}}
}
...
function saveState(){try{localStorage.setItem(STORE_KEY,JSON.stringify({inventory:state.inventory,payoutTray:state.payoutTray,stacks:Object.fromEntries(state.stacks),previousStacks:Object.fromEntries(state.previousStacks),selectedChip:state.selectedChip,dealerChip:state.dealerChip,dealerTips:state.dealerTips,history:state.history,tableCollapsed:state.tableCollapsed,halfCredit:state.halfCredit}))}catch{}}
...
roulette.innerHTML=`
  ...
  <dialog id="dealer-dialog" class="dealer-dialog"><button type="button" class="close dealer-close" aria-label="关闭">×</button><span class="rules-kicker">DEALER PREFERENCE</span><h2>与荷官对话</h2><p>赔付时请优先给我：</p><div id="dealer-chip-options" class="dealer-chip-options"></div><section class="dealer-service"><div><strong>赔付区整理</strong><span>按当前偏好等值换筹码</span></div><button type="button" id="dealer-exchange">整理筹码</button></section><section class="dealer-tip"><div><strong>给荷官小费</strong><span id="dealer-tip-total"></span></div><div id="dealer-tip-options" class="dealer-tip-options"></div></section><p id="dealer-message" aria-live="polite">荷官：请告诉我您的偏好。</p><button type="button" class="dealer-done">完成</button></dialog>`;
