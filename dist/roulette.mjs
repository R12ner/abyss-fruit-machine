...
roulette.innerHTML=`
  ...
  <main class="roulette-main">
    ...
    <div class="roulette-layout">
      ...
      <section class="betting-panel" aria-label="轮盘下注桌">
        ...
        <section class="roulette-console" aria-label="轮盘控制台"><div class="chip-rack" id="chip-rack" role="radiogroup" aria-label="选择筹码面额"></div><div class="quick-bet"><select id="quick-bet" aria-label="选择快捷下注"><option value="number-0">零号</option><optgroup label="零号旁注"><option value="split-0-1">分注 0·1</option><option value="split-0-2">分注 0·2</option><option value="split-0-3">分注 0·3</option><option value="trio-012">三数 0·1·2</option><option value="trio-023">三数 0·2·3</option><option value="basket">首四 0·1·2·3</option></optgroup><optgroup label="外部下注"><option value="red">红</option><option value="black">黑</option><option value="odd">单</option><option value="even">双</option><option value="low">1–18</option><option value="high">19–36</option><option value="dozen-1">第一打</option><option value="dozen-2">第二打</option><option value="dozen-3">第三打</option></optgroup><optgroup label="角注"><option value="corner-1">角注 1·2·4·5</option><option value="corner-2">角注 2·3·5·6</option><option value="corner-31">角注 31·32·34·35</option><option value="corner-32">角注 32·33·35·36</option></optgroup></select><button type="button" id="quick-bet-apply" class="table-action">一键下注</button></div><div class="roulette-actions"><button type="button" id="roulette-repeat" class="table-action">重复上次</button><button type="button" id="roulette-undo" class="table-action secondary">撤销</button><button type="button" id="roulette-clear" class="table-action secondary">清空</button></div><div class="roulette-totals"><span>总押</span><output id="roulette-total">0</output><small>USD</small></div><button type="button" id="roulette-spin" class="spin-button"><span>旋转</span><small>SPIN</small></button></section>
      </section>
    </div>
  ...
  <dialog id="roulette-rules" class="roulette-rules">
    ...
    <p>操作：只可使用水果机 WALLET 在商店逐枚购买筹码。线上的按钮是分注、街注或双街，四格交点是角注；右键或 Shift + 点击可收回一枚当前面额筹码。</p><p>采用 La Partage 平分规则：开出 0 时，红/黑、单/双、1–18/19–36 六种 1:1 下注立即退还一半本金，其他下注照常结算。半枚筹码会保留，凑满 1 后自动进入 1 元筹码槽。中奖筹码仍需逐枚领取。</p><button type="button" class="roulette-rules-confirm">明白了</button>
  </dialog>
...
wheelDisc.style.background=`repeating-conic-gradient(from ${-stepAngle/2}deg,transparent 0 ${stepAngle-.34}deg,#d8bd6b ${stepAngle-.34}deg ${stepAngle}deg),conic-gradient(from 0deg,${stops.join(',')})`;
...
buildShop();
buildTable();buildChipRack();$('#roulette-undo',roulette).onclick=undo;$('#roulette-clear',roulette).onclick=clearBets;$('#roulette-repeat',roulette).onclick=repeatLast;$('#quick-bet-apply',roulette).onclick=()=>adjustBet($('#quick-bet',roulette).value);$('#roulette-spin',roulette).onclick=spin;
