...
function loadState(){
  try{
    ...
    const payoutTray=Array.isArray(saved?.payoutTray)?saved.payoutTray.filter(value=>CHIP_VALUES.includes(value)):[];
    ...
    return {inventory,payoutTray,stacks,selectedChip,history:Array.isArray(saved?.history)?saved.history.slice(0,12):Array.isArray(legacy?.history)?legacy.history.slice(0,12):[],tableCollapsed:Boolean(saved?.tableCollapsed)};
  }catch{return {inventory:CHIP_VALUES.map(()=>0),payoutTray:[],stacks:new Map(),selectedChip:1,history:[],tableCollapsed:false}}
}
...
function saveState(){try{localStorage.setItem(STORE_KEY,JSON.stringify({inventory:state.inventory,payoutTray:state.payoutTray,stacks:Object.fromEntries(state.stacks),selectedChip:state.selectedChip,history:state.history,tableCollapsed:state.tableCollapsed}))}catch{}}
...
roulette.innerHTML=`
  ...
  <dialog id="roulette-rules" class="roulette-rules">
    ...
    <p>操作：先在商店逐枚购买筹码，再从下方筹码槽选择面额并点击号码或外部下注区。号码之间的细长触点是分注，四格交点是角注；“街”覆盖一排三个号码，街与街之间的小菱形是双街。右键或 Shift + 点击可收回该位置的一枚当前面额筹码。</p><p>赔率是净赢金额；中奖时系统会把返还本金和奖金换成实体筹码放在荷官赔付区，需要逐枚点击领取。赌场菜单的“自助出售筹码”可以逐枚出售筹码槽库存，或一键出售全部库存。0 不属于红黑、单双、大小、十二数区或列，因此开出 0 时这些外部下注均输。筹码库存、未领取赔付和桌面押注均会保存在当前浏览器。</p><button type="button" class="roulette-rules-confirm">明白了</button>
  </dialog>
  <dialog id="roulette-shop" class="roulette-shop">
    ...
    <div class="shop-balance"><span>水果机可用资金</span><output id="fruit-shop-balance">0 USD</output></div>
...
function chipsForAmount(amount){const chips=[];for(let index=CHIP_VALUES.length-1;index>=0;index--)while(amount>=CHIP_VALUES[index]){chips.push(CHIP_VALUES[index]);amount-=CHIP_VALUES[index]}return chips}
...
function sellChips(value,count){const index=CHIP_VALUES.indexOf(value),quantity=Math.min(count,state.inventory[index]);if(quantity<=0)return false;const amount=value*quantity,request={amount,accepted:false};window.dispatchEvent(new CustomEvent('abyss:sell-roulette-chips',{detail:request}));if(!request.accepted)return false;state.inventory[index]-=quantity;saveState();render();playChipDrop(.7);return amount}
...
$('#payout-chip-pile',roulette).onclick=event=>{const button=event.target.closest('[data-payout-index]');if(!button)return;const index=Number(button.dataset.payoutIndex),value=state.payoutTray[index];if(!CHIP_VALUES.includes(value))return;state.payoutTray.splice(index,1);state.inventory[CHIP_VALUES.indexOf(value)]++;state.selectedChip=value;saveState();render();setStatus(`已领取一枚 ${money(value)} 筹码`);playChipDrop()};
