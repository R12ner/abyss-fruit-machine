...
export function createFruitGame({wallet,openLobby}){
if(fruitInstance)return fruitInstance;
...
subsidyDialog.innerHTML='<span class="eyebrow">HOUSE SUPPORT</span><h2>补贴待领取</h2><p>总金额已归零。领取后，游戏厅会把 10 USD 补贴立即存入钱包。</p><button type="button" class="subsidy-confirm">领取 10 USD</button>';
...
subsidyDialog.querySelector('button').onclick=()=>{
  ...
  saveGameState();subsidyDialog.close();update();status.textContent='已领取破产补贴 10 USD';
};
...
setLed('multiplier-led',0,3);
  ...
  wallet.connect({
    balance:()=>game.snapshot().wallet,
    spend(amount){const spent=game.spend(amount);if(!spent)return 0;saveGameState();update();status.textContent=`已在赌场消费 ${spent} USD`;return spent},
    deposit(amount){const deposited=game.deposit(amount);if(!deposited)return 0;saveGameState();update();status.textContent=`赌场柜台返还 ${deposited} USD`;return deposited}
  });
  let menuButton=document.querySelector('.casino-switch');
  if(!menuButton){menuButton=document.createElement('button');menuButton.type='button';menuButton.className='arcade-button utility casino-switch';menuButton.setAttribute('aria-label','返回赌场菜单');menuButton.textContent='♣';document.querySelector('.header-right')?.prepend(menuButton)}
  menuButton.onclick=()=>openLobby('games');
  fruitInstance=Object.freeze({
    enter(){document.body.classList.add('mode-fruit');document.body.classList.remove('mode-roulette');update()},
    leave(){stopAllKeyHolds();document.body.classList.remove('mode-fruit')}
  });
