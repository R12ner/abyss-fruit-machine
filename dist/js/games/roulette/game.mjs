...
function loadState(){
  try{
    ...
    const dealerChip=saved?.dealerChip==='max'||CHIP_VALUES.includes(saved?.dealerChip)?saved.dealerChip:'max';
    return {inventory,payoutTray,stacks,previousStacks,selectedChip,dealerChip,dealerTips:Number.isFinite(saved?.dealerTips)&&saved.dealerTips>=0?saved.dealerTips:0,dealerIntel:restoreDealerIntel(saved?.dealerIntel),history:Array.isArray(saved?.history)?saved.history.slice(0,12):Array.isArray(legacy?.history)?legacy.history.slice(0,12):[],tableCollapsed:Boolean(saved?.tableCollapsed),halfCredit:saved?.halfCredit===.5?.5:0};
  }catch{return {inventory:CHIP_VALUES.map(()=>0),payoutTray:[],stacks:new Map(),previousStacks:new Map(),selectedChip:1,dealerChip:'max',dealerTips:0,dealerIntel:null,history:[],tableCollapsed:false,halfCredit:0}}
}

let rouletteInstance=null;
export function createRouletteGame({wallet,openLobby}){
if(rouletteInstance)return rouletteInstance;
...
function saveState(){try{localStorage.setItem(STORE_KEY,JSON.stringify({inventory:state.inventory,payoutTray:state.payoutTray,stacks:Object.fromEntries(state.stacks),previousStacks:Object.fromEntries(state.previousStacks),selectedChip:state.selectedChip,dealerChip:state.dealerChip,dealerTips:state.dealerTips,dealerIntel:state.dealerIntel,history:state.history,tableCollapsed:state.tableCollapsed,halfCredit:state.halfCredit}))}catch{}}
...
roulette.innerHTML=`
  ...
  <dialog id="dealer-dialog" class="dealer-dialog"><button type="button" class="close dealer-close" aria-label="关闭">×</button><span class="rules-kicker">DEALER PREFERENCE</span><h2>与荷官对话</h2><p>赔付时请优先给我：</p><div id="dealer-chip-options" class="dealer-chip-options"></div><section class="dealer-service"><div><strong>赔付区整理</strong><span>按当前偏好等值换筹码</span></div><button type="button" id="dealer-exchange">整理筹码</button></section><section class="dealer-tip"><div><strong>给荷官小费</strong><span id="dealer-tip-total"></span></div><small>小费越大，偶遇内幕彩蛋的概率越高；消息不保证为真。</small><div id="dealer-tip-options" class="dealer-tip-options"></div></section><aside id="dealer-intel" class="dealer-intel" hidden><span>♣ INSIDER TIP · 下一局有效</span><strong id="dealer-intel-message"></strong></aside><p id="dealer-message" aria-live="polite">${DEALER_SAYINGS.welcome}</p><button type="button" class="dealer-done">完成</button></dialog>
  <dialog id="roulette-subsidy" class="subsidy-dialog roulette-subsidy"><span class="rules-kicker">HOUSE PROTECTION</span><h2>破产保护</h2><p>赌场钱包与轮盘资产均已归零。领取一枚 1 USD 筹码继续游戏。</p><button type="button" class="subsidy-confirm">领取 1 USD 筹码</button></dialog>`;
document.body.append(roulette);
$('.casino-menu-button',roulette).onclick=()=>openLobby('games');
...
if(audioNote&&!settingsDialog.querySelector('[data-roulette-hints]')){
  const setting=document.createElement('label');setting.className='key-hint-setting';setting.dataset.rouletteHints='';
  setting.innerHTML='<span><b>轮盘落点提示</b><small>显示分注、角注和双街的小圆点</small></span><input type="checkbox" role="switch" aria-label="显示轮盘落点提示"><i aria-hidden="true"></i>';
  audioNote.before(setting);const toggle=setting.querySelector('input');toggle.checked=showRouletteHints;
  toggle.onchange=()=>{showRouletteHints=toggle.checked;document.body.classList.toggle('hide-roulette-bet-hints',!showRouletteHints);try{localStorage.setItem(ROULETTE_HINT_KEY,showRouletteHints?'on':'off')}catch{}};
}
document.body.classList.toggle('hide-roulette-bet-hints',!showRouletteHints);
...
function chipsForAmount(amount,preferred=state.dealerChip){const chips=[];if(preferred!=='max'&&CHIP_VALUES.includes(preferred))while(amount>=preferred){chips.push(preferred);amount-=preferred}for(let index=CHIP_VALUES.length-1;index>=0;index--)while(amount>=CHIP_VALUES[index]){chips.push(CHIP_VALUES[index]);amount-=CHIP_VALUES[index]}if(amount>=.5)chips.push(.5);return chips}
...
function playSecretSound(){soundBurst([330,494,659,988],.26,1,'sine')}
...
function spin(){
  if(state.busy)return;const stake=totalBet();if(!stake){setStatus('请先在桌面放置筹码');return}let result;const dealerIntel=state.dealerIntel;
  try{result=dealerIntel?.result??secureNumber(37)}catch{setStatus('安全随机数暂不可用，本轮未扣款');return}
  state.dealerIntel=null;
  state.previousStacks=new Map([...state.stacks].map(([id,stack])=>[id,[...stack]]));state.busy=true;state.lastResult=null;saveState();render();setStatus('停止下注 · 轮盘旋转中');const index=ROULETTE_SEQUENCE.indexOf(result),base=Math.ceil(state.wheelRotation/360)*360+(REDUCED_MOTION.matches?360:1800);state.wheelRotation=base-index*stepAngle;
  ...
  setTimeout(()=>{if(tickTimer)clearInterval(tickTimer);wheel.classList.remove('spinning');ball.classList.remove('orbiting');const settlement=settleBets(state.bets,result);state.payoutTray.push(...chipsForAmount(settlement.returned));state.history.unshift(result);state.history=state.history.slice(0,12);state.lastResult=result;state.busy=false;state.bets.clear();state.stacks.clear();state.actions=[];saveState();render();const reaction=dealerIntel?.truthful===false?dealerIntel.revealMessage:'';let message;if(settlement.returned){const winLabels=settlement.wins.map(win=>win.label).join('、');message=`开出 ${result} · ${winLabels} 命中 · 请逐枚领取 ${money(settlement.returned)} 筹码`;playTone(720,.35)}else{message=`开出 ${result} · 本轮未中奖`;playTone(150,.35)}if(reaction){message+=` · ${reaction}`;$('#dealer-message',dealerDialog).textContent=reaction}setStatus(message)},duration);
}
...
function render(){
  ...
  const dealerTalk=$('#dealer-talk',roulette);dealerTalk.classList.toggle('has-intel',Boolean(state.dealerIntel));dealerTalk.textContent=state.dealerIntel?'荷官有话':'与荷官对话';
  ...
  $('#roulette-undo',roulette).disabled=state.busy||!state.actions.length;$('#roulette-clear',roulette).disabled=state.busy||!state.bets.size;$('#roulette-repeat',roulette).disabled=state.busy||state.bets.size>0||!state.previousStacks.size;$('#quick-bet-apply',roulette).disabled=state.busy;$('#roulette-spin',roulette).disabled=state.busy||!state.bets.size;$('#roulette-refill',roulette).hidden=inventoryTotal()>0||state.bets.size>0||state.payoutTray.length>0||state.busy;renderHistory();renderPayout();renderSummary();maybeShowRouletteProtection();
}

function fruitBalance(){return wallet.balance()}
const shop=$('#roulette-shop',roulette);document.body.append(shop);const dealerDialog=$('#dealer-dialog',roulette);document.body.append(dealerDialog);const rouletteSubsidyDialog=$('#roulette-subsidy',roulette);document.body.append(rouletteSubsidyDialog);const shopStatus=$('#shop-status',shop);let shopMode='buy';
function rouletteBankruptcyEligible(){return isRouletteBankrupt({walletBalance:wallet.balance(),inventoryTotal:inventoryTotal(),betTotal:totalBet(),payoutTotal:payoutTotal(),halfCredit:state.halfCredit,busy:state.busy})}
function maybeShowRouletteProtection(){if(!roulette.hidden&&rouletteBankruptcyEligible()&&!rouletteSubsidyDialog.open)rouletteSubsidyDialog.showModal()}
rouletteSubsidyDialog.addEventListener('cancel',event=>event.preventDefault());
rouletteSubsidyDialog.querySelector('button').onclick=()=>{if(!rouletteBankruptcyEligible())return;state.inventory[0]++;state.selectedChip=1;saveState();rouletteSubsidyDialog.close();render();setStatus('已领取破产保护 · 获得一枚 1 USD 筹码');playChipDrop()};
...
function sellChips(value,count){const index=CHIP_VALUES.indexOf(value),quantity=Math.min(count,state.inventory[index]);if(quantity<=0)return false;const amount=value*quantity;if(wallet.deposit(amount)!==amount)return false;state.inventory[index]-=quantity;saveState();render();playCashRegister();return amount}
...
function setDealerPreference(value){state.dealerChip=value;saveState();renderDealerDialog();$('#dealer-message',dealerDialog).textContent=value==='max'?DEALER_SAYINGS.preferenceMax:formatDealerSaying('preferenceValue',{value:money(value)})}
function renderDealerDialog(){
  const options=$('#dealer-chip-options',dealerDialog);options.replaceChildren();
  const maxButton=document.createElement('button');maxButton.type='button';maxButton.className='dealer-max';maxButton.classList.toggle('selected',state.dealerChip==='max');maxButton.setAttribute('aria-pressed',String(state.dealerChip==='max'));maxButton.setAttribute('aria-label','优先使用最大面额筹码');maxButton.innerHTML='<strong>MAX</strong><span>最大面额</span>';maxButton.onclick=()=>setDealerPreference('max');options.append(maxButton);
  CHIP_VALUES.forEach(value=>{const button=document.createElement('button');button.type='button';button.dataset.dealerChip=value;button.classList.toggle('selected',state.dealerChip===value);button.setAttribute('aria-pressed',String(state.dealerChip===value));button.setAttribute('aria-label',`优先获得 ${money(value)} 面值筹码`);button.innerHTML=chipArt(value,'dealer-chip')+`<span>${money(value)}</span>`;button.onclick=()=>setDealerPreference(value);options.append(button)});
  const total=payoutTotal(),exchange=$('#dealer-exchange',dealerDialog),tipOptions=$('#dealer-tip-options',dealerDialog);exchange.disabled=total<=0;tipOptions.replaceChildren();
  const counts=new Map();state.payoutTray.forEach(value=>counts.set(value,(counts.get(value)||0)+1));[...counts].sort((a,b)=>b[0]-a[0]).forEach(([value,count])=>{const button=document.createElement('button');button.type='button';button.dataset.tipChip=value;button.setAttribute('aria-label',`给荷官一枚 ${money(value)} 筹码小费`);button.innerHTML=chipArt(value,'dealer-tip-chip')+`<span>${money(value)} × ${money(count)}</span>`;tipOptions.append(button)});
  if(!tipOptions.children.length)tipOptions.innerHTML='<span class="dealer-empty">赔付区没有可用于小费的筹码</span>';$('#dealer-tip-total',dealerDialog).textContent=`累计小费 ${money(state.dealerTips)} USD`;
  const intel=$('#dealer-intel',dealerDialog);intel.hidden=!state.dealerIntel;$('#dealer-intel-message',dealerDialog).textContent=state.dealerIntel?.message||'';
}
...
$('#sell-all-chips',shop).onclick=()=>{const total=inventoryTotal();if(!total)return;if(wallet.deposit(total)!==total){shopStatus.textContent='当前暂时无法出售筹码。';return}state.inventory.fill(0);saveState();render();renderShop();shopStatus.textContent=`已一键出售全部筹码，赌场钱包增加 ${money(total)} USD。`;playCashRegister()};
$('.roulette-shop-button',roulette).onclick=()=>openShop('buy');$('.roulette-settings-button',roulette).onclick=()=>document.querySelector('#settings')?.click();$('#roulette-refill',roulette).onclick=()=>openShop('buy');$('.roulette-shop-close',shop).onclick=()=>shop.close();
...
$('#dealer-exchange',dealerDialog).onclick=()=>{const total=payoutTotal();if(total<=0)return;state.payoutTray=chipsForAmount(total);saveState();render();renderDealerDialog();$('#dealer-message',dealerDialog).textContent=state.dealerChip==='max'?formatDealerSaying('exchangeMax',{total:money(total)}):formatDealerSaying('exchangePreferred',{total:money(total),preferred:money(state.dealerChip)});playExchangeSound()};
$('#dealer-tip-options',dealerDialog).onclick=event=>{const button=event.target.closest('[data-tip-chip]');if(!button)return;const value=Number(button.dataset.tipChip),index=state.payoutTray.indexOf(value);if(index<0)return;state.payoutTray.splice(index,1);state.dealerTips+=value;let intel=null;if(!state.dealerIntel)try{intel=createDealerIntel(value,secureNumber)}catch{}if(intel)state.dealerIntel=intel;saveState();render();renderDealerDialog();$('#dealer-message',dealerDialog).textContent=state.dealerIntel&&!intel?DEALER_SAYINGS.alreadyShared:intel?DEALER_SAYINGS.secretLead:formatDealerSaying('tipThanks',{value:money(value)});if(intel)playSecretSound();else playChipPickup()};
$('#table-collapse',roulette).onclick=()=>{state.tableCollapsed=!state.tableCollapsed;saveState();render()};
const rules=$('#roulette-rules',roulette);$('.roulette-rules-button',roulette).onclick=()=>rules.showModal();$('.roulette-rules-close',rules).onclick=()=>rules.close();$('.roulette-rules-confirm',rules).onclick=()=>rules.close();document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!rules.open&&document.body.classList.contains('mode-roulette'))openLobby('games')});render();
rouletteInstance=Object.freeze({
  enter(){roulette.hidden=false;document.body.classList.add('mode-roulette');document.body.classList.remove('mode-fruit');startRouletteMusic();render()},
  leave(){saveState();stopRouletteMusic();roulette.hidden=true;document.body.classList.remove('mode-roulette');[shop,dealerDialog,rules,rouletteSubsidyDialog].forEach(dialog=>{if(dialog.open)dialog.close()})},
  lobbyActions:[{id:'sell-roulette-chips',label:'自助出售筹码',className:'gateway-sell',run:()=>openShop('sell')}]
});
