...
assert.equal(g.settleGamble().win,100);assert.equal(g.snapshot().credit,190);
assert.equal(g.settleGamble(),null);
assert.equal(g.startGamble('small',()=>7),true); // 8: small loses
assert.equal(g.settleGamble().won,false);assert.equal(g.snapshot().credit,90);
...
assert.equal(x.snapshot().credit,90+1600);assert.equal(x.snapshot().risk,0);
assert.equal(x.startGamble('big'),false);
const y=createGame();y.adjust(7,10);y.start(()=>6,none);y.settle();assert.equal(y.collect(),true);
assert.equal(y.snapshot().credit,140);assert.equal(y.collect(),false);assert.equal(y.startGamble('small'),false);
