...
for(const type of ['bigTriple','smallTriple']){
 ...
 assert.equal(g.snapshot().credit,94);
 ...
 assert.equal(g.snapshot().credit,94);assert.equal(g.snapshot().win,expected+result.baseWin);assert.equal(g.settle(),null);
...
assert.equal(g.snapshot().jackpot,1003);assert.equal(g.snapshot().credit,47);
...
assert.equal(g.snapshot().credit,47);assert.equal(g.snapshot().win,1003);assert.equal(g.snapshot().risk,1003);
assert.equal(g.snapshot().jackpot,1000);assert.equal(g.settle(),null);
g.startGamble('small',()=>13);g.settleGamble();
assert.equal(g.snapshot().credit,47);assert.equal(g.snapshot().win,0);assert.equal(g.snapshot().jackpot,1000);
...
assert.equal(b.win,1051);assert.equal(both.snapshot().credit,90);assert.equal(both.snapshot().win,1051);
...
assert.equal(failure.snapshot().credit,100);assert.equal(failure.snapshot().jackpot,1000);
