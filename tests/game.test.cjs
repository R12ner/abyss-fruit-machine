...
const createGame=()=>{const g=rawCreateGame(),start=g.start;g.start=(choose)=>start(choose,()=>({type:null,indices:[]}));return g};
...
for (const [index,[symbol,multiplier]] of route.entries()) {
  ...
  assert.equal(game.snapshot().credit,964);
  ...
  assert.equal(game.snapshot().credit,964+result.win);
...
for(const [symbol,stake,index,win] of examples){
  ...
  assert.equal(g.settle().win,win);assert.equal(g.snapshot().credit,1000-stake+win);
}
const g=createGame();
...
for(let i=0;i<10;i++){g.adjust(7,99);g.start(()=>4);g.settle()}
...
assert.equal(safe.snapshot().credit,1000);assert.equal(safe.snapshot().busy,false);
