...
for(const type of ['bigTriple','smallTriple']){
 ...
 assert.equal(g.snapshot().credit,94+expected+result.baseWin);assert.equal(g.settle(),null);
...
assert.equal(g.snapshot().credit,1050);assert.equal(g.snapshot().risk,1003);
assert.equal(g.snapshot().jackpot,1000);assert.equal(g.settle(),null);
g.startGamble('small',()=>13);g.settleGamble();
assert.equal(g.snapshot().credit,47);assert.equal(g.snapshot().jackpot,1000);
...
assert.equal(b.win,1051);assert.equal(both.snapshot().credit,1141);
