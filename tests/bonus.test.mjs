...
for(const type of ['gift','train']){
 ...
 assert.equal(g.snapshot().credit,20);
 ...
 assert.equal(g.snapshot().credit,20);assert.equal(g.snapshot().win,1200+expected);
 assert.equal(g.snapshot().risk,r.win);assert.equal(g.settle(),null);
}
const g=createGame();g.adjust(7,10);g.start(()=>6,none);g.settle();
assert.equal(g.snapshot().credit,90);assert.equal(g.snapshot().win,50);
g.adjust(7,1);assert.equal(g.startGamble('small'),false);g.clear();
assert.throws(()=>g.startGamble('small',()=>{throw Error('rng')}));
assert.equal(g.snapshot().credit,90);assert.equal(g.snapshot().risk,50);
assert.equal(g.startGamble('small',()=>6),true); // 7: small wins
assert.equal(g.snapshot().credit,90);assert.equal(g.snapshot().win,0);assert.equal(g.adjust(0,1),false);
...
assert.equal(g.settleGamble().win,100);assert.equal(g.snapshot().credit,90);assert.equal(g.snapshot().win,100);
assert.equal(g.settleGamble(),null);
assert.equal(g.startGamble('small',()=>7),true); // 8: small loses
assert.equal(g.settleGamble().won,false);assert.equal(g.snapshot().credit,90);assert.equal(g.snapshot().win,0);
...
assert.equal(x.snapshot().credit,90);assert.equal(x.snapshot().win,1600);assert.equal(x.snapshot().risk,0);
assert.equal(x.startGamble('big'),false);
const y=createGame();y.adjust(7,10);y.start(()=>6,none);y.settle();assert.equal(y.collect(),true);
assert.equal(y.snapshot().credit,90);assert.equal(y.snapshot().win,50);assert.equal(y.collect(),false);assert.equal(y.startGamble('small'),false);
const z=createGame();z.adjust(7,10);
assert.throws(()=>z.start(()=>6,()=>{throw Error('bonus rng')}));assert.equal(z.snapshot().credit,100);
